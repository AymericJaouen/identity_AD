...
# ╔═══════════════════════════════════════════════════════════════════╗
# ║             Initialization: Version & Module Check                ║
# ╚═══════════════════════════════════════════════════════════════════╝
...
$msaAccounts = Get-ADServiceAccount -Server $domainName -Filter {ObjectClass -eq "msDS-ManagedServiceAccount"} | Select-Object -ExpandProperty SamAccountName
$gmsaAccounts = Get-ADServiceAccount -Server $domainName -Filter {ObjectClass -eq "msDS-GroupManagedServiceAccount"} | Select-Object -ExpandProperty SamAccountName
$nonExpiringUsers = Get-ADUser -Server $domainName -Filter {PasswordNeverExpires -eq $true -and Enabled -eq $true} | Select-Object -ExpandProperty SamAccountName

$patternMatchedUsers = @()
foreach ($pattern in $UserServiceAccountNamesLike) {
    $patternMatchedUsers += Get-ADUser -Server $domainName -Filter "SamAccountName -like '$pattern'" | Select-Object -ExpandProperty SamAccountName
}
